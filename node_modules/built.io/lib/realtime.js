...
function setupAppListeners(eventName, callback){
  switch(eventName){
    ...
    case 'reconnect':
    if(this.getMetaMap())
      this.getMetaMap()['reconnect'] = callback
    break;
    case 'disconnect':
    if(this.getMetaMap())
      this.getMetaMap()['disconnect'] = callback
    break;
    case 'error':
    if(this.getMetaMap())
      this.getMetaMap()['error'] = callback 
    break;
  }
}
...
function storeSubscriberInMap(app, channel, subsriberStr) {
  ...
  app.getMetaMap()[subsriberStr] = function(socket) { //e.g., subscription.person.blt123.update
    ...
    socket.emit('subscribe', {
      ...
      if (error && app.getMetaMap().errorHandler) { // if there is an error in subscription (no permission to read on the entity), 'error' will contain details
        app.getMetaMap().errorHandler(error);
      }
      deferred.resolve();
    });
...
function attachListeners(newApp) {
  ...
  socket.once('connect', function() {
    if(newApp.getMetaMap().connect)
      newApp.getMetaMap().connect();

    onConnect(newApp);
  });
  socket.on('reconnect', function() {
    console.log("reconnecting socket");
    if(newApp.getMetaMap().reconnect)
      newApp.getMetaMap().reconnect();
  ...
  socket.on('disconnect', function(){
    ...
    if(newApp.getMetaMap().disconnect)
      newApp.getMetaMap().disconnect();
  });
  socket.on('error', function(error) {
    ...
    if(newApp.getMetaMap().error)
      newApp.getMetaMap().error(error);
  });
...
function onConnect(newApp){
  ...
  authPromise
    ...
    .then(function() {
      if (!newApp.isSocketAuthStarted()) { // Authentication process is not going to happen now, so tries to flush the operation
...
* user.fetch()
* .then(function(user){
*   console.log(user.getPresence()); // Extracts '_presence' property from user object
* })
  @return {Promise<Presence>}
*/
